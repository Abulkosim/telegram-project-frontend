    ...
    <div class="w-full max-w-md z-10">
      <div class="bg-gray-800 bg-opacity-80 rounded-lg overflow-hidden shadow-2xl">
        <div class="p-8">
          ...
          <LoginForm v-if="!isRegister" @submit="handleSubmit" :is-loading="isLoading" />
          <RegisterForm v-else @submit="handleSubmit" :is-loading="isLoading" />

          <AuthToggle :is-register="isRegister" @toggle="toggleForm" />
        </div>
      </div>
    </div>
  </div>
...
import AuthToggle from '@/features/auth/ui/AuthToggle.vue'
...
const handleSubmit = async (formData: any) => {
  ...
  try {
    if (isRegister.value) {
      await useAuth().register(formData.username, formData.email, formData.password)
    } else {
      await useAuth().login(formData.email, formData.password)
    }
  } catch (error) {
